...
#print(corpus, wordtoid, idtoword)
...
def create_to_matrix(corpus,vocab_size,window_size=1):
	...
	return co_matrix#修正箇所(returnがforの中に入っていた)
...
def create_contexts_target(corpus,window_size=1):
	...
	return np.array(contexts),np.array(target)#重要
...
def analogy(a,b,c,wordtoid,idtoword,word_matrix,top=5,answer=None):
	for word in (a,b,c):#重要
		if word not in wordtoid:
			print('%s is not fonud'% word)
			return#重要

	print('\n[analogy]'+a+':'+b+'='+c+':?')
	a_vec,b_vec,c_vec=word_matrix[wordtoid[a]],word_matrix[wordtoid[b]],word_matrix[wordtoid[c]]
	query_vec=b_vec-a_vec+c_vec
	query_vec=normalize(query_vec)#重要

	similarity=np.dot(word_matrix,query_vec)
	#↑各候補とかけあわせて値を算出することで、類似する可能性の高いワードをあぶり出す

	if answer is not None:
		print('==>'+answer+':'+str(np.dot(word_matrix[wordtoid[answer]],query_vec)))	

	count=0
	for i in (-1*similarity).argsort():
		if np.isnan(similarity[i]):#ベクトルが存在するか
			continue
...
def normalize(x):
	if x.ndim==2:
		s=np.sqrt((x*x).sum(1))#重要
		x/=s.reshape((s.reshape[0],1))#重要
	elif x.ndim==1:
		s=np.sqrt((x*x).sum())
		x/=s
	return x
